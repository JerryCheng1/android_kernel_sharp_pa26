...
#ifdef SHIRDA_CONFIG_H
#define TTYHS_DEVFILE_NAME		SHIRDA_TTY_DEVNAME

#define TTYHS_DEVFILE			"/dev/"TTYHS_DEVFILE_NAME

#else
#define PAR_TTYDEVNAME	"/sys/module/shirda_msm_kdrv/parameters/ttydevname"
inline static char *get_tty_devname() {
	static char ss[64];
	int fd;
	int n;

	fd = open(PAR_TTYDEVNAME, O_RDONLY);
	if (fd < 0) return NULL;
	n = read(fd, ss, 63);
	close(fd);
	if (n > 8) {
