/* drivers/sharp/shdisp/data/shdisp_bl71y6_data_default.h  (Display Driver)
 *
 * Copyright (C) 2011 SHARP CORPORATION
 *
 * This software is licensed under the terms of the GNU General Public
 * License version 2, as published by the Free Software Foundation, and
 * may be copied, distributed, and modified under those terms.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 */

/* ------------------------------------------------------------------------- */
/* SHARP DISPLAY DRIVER FOR KERNEL MODE                                      */
/* ------------------------------------------------------------------------- */
#ifndef SHDISP_BL71Y6_DATA_DEFAULT_H
#define SHDISP_BL71Y6_DATA_DEFAULT_H

/* ------------------------------------------------------------------------- */
/* INCLUDE FILES                                                             */
/* ------------------------------------------------------------------------- */
#include "../shdisp_bl71y6.h"

/* ------------------------------------------------------------------------- */
/* MACROS                                                                    */
/* ------------------------------------------------------------------------- */
#define SHDISP_BKL_FIX_TBL_NUM                  (256)
#define SHDISP_BKL_AUTO_STEP_NUM                (253)
#define SHDISP_BKL_AUTO_OPT_TBL_NUM             (24)
#define SHDISP_BKL_EMERGENCY_LIMIT_AUTO         (135)
#define SHDISP_BKL_EMERGENCY_LIMIT_FIX          (0x1A)
#define SHDISP_BKL_PWM_LOWER_LIMIT              (0x0010)
#define SHDISP_BKL_PWM_UPPER_LIMIT              (0x03E8)
#define SHDISP_BKL_CURRENT_LOWER_LIMIT          (0)
#define SHDISP_BKL_CURRENT_UPPER_LIMIT          (190)

#define SHDISP_INT_ENABLE_GFAC                  (SHDISP_BDIC_INT_GFAC_ALS | \
                                                 SHDISP_BDIC_INT_GFAC_DET | \
                                                 SHDISP_BDIC_INT_GFAC_I2C_ERR | \
                                                 SHDISP_BDIC_INT_GFAC_OPTSEL)

#define SHDISP_LUX_CHANGE_LEVEL1                (0x0B)
#define SHDISP_LUX_CHANGE_LEVEL2                (0x01)

#define SHDISP_TRI_LED_COLOR_TBL_NUM            (11)
#define SHDISP_COL_VARI_KIND                    (3)
#define SHDISP_HANDSET_COLOR_WHITE              (0x01)
#define SHDISP_HANDSET_COLOR_PINK               (0x02)
#define SHDISP_HANDSET_COLOR_BLACK              (0x06)
#define SHDISP_TRI_LED_ANIME_2PAGE              (2)
#define SHDISP_TRI_LED_ANIME_3PAGE              (3)

/* ------------------------------------------------------------------------- */
/* MACROS(Register Value)                                                    */
/* ------------------------------------------------------------------------- */
#define BDIC_REG_VO2_PFML_VAL                   (0x17)
#define BDIC_REG_VO2_PWML_VAL                   (0x17)
#define BDIC_REG_VO2_PWMH_VAL                   (0x17)
#define BDIC_REG_M1LED_VAL                      (0x5C)
#define BDIC_REG_M2LED_VAL                      (0x5C)
#define BDIC_REG_SYSTEM1_BKL                    (0x03)
#define BDIC_REG_DCDC1_CLIMT_2_VAL              (0xC0)
#define BDIC_REG_DCDC1_GM_VAL                   (0x08)
#define BDIC_REG_DCDC1_OSC_2_VAL                (0x01)

#define BDIC_REG_AR_HI_TH_L_L_VAL               (0xFF)
#define BDIC_REG_AR_HI_TH_L_H_VAL               (0xFF)
#define BDIC_REG_AR_LO_TH_M_L_VAL               (0xE0)
#define BDIC_REG_AR_LO_TH_M_H_VAL               (0x2E)
#define BDIC_REG_AR_HI_TH_M_L_VAL               (0xFF)
#define BDIC_REG_AR_HI_TH_M_H_VAL               (0xFF)
#define BDIC_REG_AR_LO_TH_H_L_VAL               (0xFF)
#define BDIC_REG_AR_LO_TH_H_H_VAL               (0xFF)
#define BDIC_REG_AR_RANGE_L_VAL                 (0x1C)
#define BDIC_REG_AR_RANGE_M_VAL                 (0x1E)
#define BDIC_REG_AR_RANGE_H_VAL                 (0x1E)
#define BDIC_REG_AR_RANGE_L_VAL_PS_ON           (0x24)
#define BDIC_REG_AR_RANGE_M_VAL_PS_ON           (0x26)
#define BDIC_REG_AR_RANGE_H_VAL_PS_ON           (0x26)
#define BDIC_REG_DATA_SHIFT_M_VAL               (0x64)
#define BDIC_REG_DATA_SHIFT_H_VAL               (0x06)

#define BDIC_REG_AR_CTRL_VAL                    (0xDE)
#define BDIC_REG_GPIO_2_VAL                     (0x04)

#define BDIC_REG_ALS_ADJ0_L_DEFAULT_A           (0x5C)
#define BDIC_REG_ALS_ADJ0_H_DEFAULT_A           (0x3F)
#define BDIC_REG_ALS_ADJ1_L_DEFAULT_A           (0x5C)
#define BDIC_REG_ALS_ADJ1_H_DEFAULT_A           (0x3F)
#define BDIC_REG_ALS_SHIFT_DEFAULT_A            (0x03)
#define BDIC_REG_CLEAR_OFFSET_DEFAULT_A         (0x00)
#define BDIC_REG_IR_OFFSET_DEFAULT_A            (0x00)

#define BDIC_REG_ALS_ADJ0_L_DEFAULT_B           (0xAC)
#define BDIC_REG_ALS_ADJ0_H_DEFAULT_B           (0x0C)
#define BDIC_REG_ALS_ADJ1_L_DEFAULT_B           (0x00)
#define BDIC_REG_ALS_ADJ1_H_DEFAULT_B           (0x00)
#define BDIC_REG_ALS_SHIFT_DEFAULT_B            (0x03)
#define BDIC_REG_CLEAR_OFFSET_DEFAULT_B         (0x00)
#define BDIC_REG_IR_OFFSET_DEFAULT_B            (0x00)

#define BDIC_REG_OPT_TH_SHIFT_1_0_VAL           (0x44)
#define BDIC_REG_OPT_TH_SHIFT_3_2_VAL           (0x44)
#define BDIC_REG_OPT_TH_SHIFT_4_5_VAL           (0x44)
#define BDIC_REG_OPT_TH_SHIFT_6_7_VAL           (0x44)
#define BDIC_REG_OPT_TH_SHIFT_8_9_VAL           (0x44)
#define BDIC_REG_OPT_TH_SHIFT_11_10_VAL         (0x44)
#define BDIC_REG_OPT_TH_SHIFT_13_12_VAL         (0x44)
#define BDIC_REG_OPT_TH_SHIFT_15_14_VAL         (0x44)
#define BDIC_REG_OPT_TH_SHIFT_17_16_VAL         (0x44)
#define BDIC_REG_OPT_TH_SHIFT_19_18_VAL         (0x44)
#define BDIC_REG_OPT_TH_SHIFT_21_20_VAL         (0x44)
#define BDIC_REG_OPT_TH_SHIFT_23_22_VAL         (0x44)

#define BDIC_REG_OPT0_LT_L_VAL                  (0x00)
#define BDIC_REG_OPT0_LT_H_VAL                  (0x00)
#define BDIC_REG_OPT0_HT_L_VAL                  (0x00)
#define BDIC_REG_OPT0_HT_H_VAL                  (0x00)
#define BDIC_REG_OPT1_LT_L_VAL                  (0x00)
#define BDIC_REG_OPT1_LT_H_VAL                  (0x00)
#define BDIC_REG_OPT1_HT_L_VAL                  (0x01)
#define BDIC_REG_OPT1_HT_H_VAL                  (0x00)
#define BDIC_REG_OPT2_LT_L_VAL                  (0x01)
#define BDIC_REG_OPT2_LT_H_VAL                  (0x00)
#define BDIC_REG_OPT2_HT_L_VAL                  (0x01)
#define BDIC_REG_OPT2_HT_H_VAL                  (0x00)
#define BDIC_REG_OPT3_LT_L_VAL                  (0x01)
#define BDIC_REG_OPT3_LT_H_VAL                  (0x00)
#define BDIC_REG_OPT3_HT_L_VAL                  (0x01)
#define BDIC_REG_OPT3_HT_H_VAL                  (0x00)
#define BDIC_REG_OPT4_LT_L_VAL                  (0x02)
#define BDIC_REG_OPT4_LT_H_VAL                  (0x00)
#define BDIC_REG_OPT4_HT_L_VAL                  (0x05)
#define BDIC_REG_OPT4_HT_H_VAL                  (0x00)
#define BDIC_REG_OPT5_LT_L_VAL                  (0x04)
#define BDIC_REG_OPT5_LT_H_VAL                  (0x00)
#define BDIC_REG_OPT5_HT_L_VAL                  (0x08)
#define BDIC_REG_OPT5_HT_H_VAL                  (0x00)
#define BDIC_REG_OPT6_LT_L_VAL                  (0x07)
#define BDIC_REG_OPT6_LT_H_VAL                  (0x00)
#define BDIC_REG_OPT6_HT_L_VAL                  (0x0C)
#define BDIC_REG_OPT6_HT_H_VAL                  (0x00)
#define BDIC_REG_OPT7_LT_L_VAL                  (0x0A)
#define BDIC_REG_OPT7_LT_H_VAL                  (0x00)
#define BDIC_REG_OPT7_HT_L_VAL                  (0x12)
#define BDIC_REG_OPT7_HT_H_VAL                  (0x00)
#define BDIC_REG_OPT8_LT_L_VAL                  (0x10)
#define BDIC_REG_OPT8_LT_H_VAL                  (0x00)
#define BDIC_REG_OPT8_HT_L_VAL                  (0x1C)
#define BDIC_REG_OPT8_HT_H_VAL                  (0x00)
#define BDIC_REG_OPT9_LT_L_VAL                  (0x18)
#define BDIC_REG_OPT9_LT_H_VAL                  (0x00)
#define BDIC_REG_OPT9_HT_L_VAL                  (0x34)
#define BDIC_REG_OPT9_HT_H_VAL                  (0x00)
#define BDIC_REG_OPT10_LT_L_VAL                 (0x2A)
#define BDIC_REG_OPT10_LT_H_VAL                 (0x00)
#define BDIC_REG_OPT10_HT_L_VAL                 (0x53)
#define BDIC_REG_OPT10_HT_H_VAL                 (0x00)
#define BDIC_REG_OPT11_LT_L_VAL                 (0x48)
#define BDIC_REG_OPT11_LT_H_VAL                 (0x00)
#define BDIC_REG_OPT11_HT_L_VAL                 (0x80)
#define BDIC_REG_OPT11_HT_H_VAL                 (0x00)
#define BDIC_REG_OPT12_LT_L_VAL                 (0x6F)
#define BDIC_REG_OPT12_LT_H_VAL                 (0x00)
#define BDIC_REG_OPT12_HT_L_VAL                 (0xD0)
#define BDIC_REG_OPT12_HT_H_VAL                 (0x00)
#define BDIC_REG_OPT13_LT_L_VAL                 (0xB1)
#define BDIC_REG_OPT13_LT_H_VAL                 (0x00)
#define BDIC_REG_OPT13_HT_L_VAL                 (0x33)
#define BDIC_REG_OPT13_HT_H_VAL                 (0x01)
#define BDIC_REG_OPT14_LT_L_VAL                 (0xB1)
#define BDIC_REG_OPT14_LT_H_VAL                 (0x00)
#define BDIC_REG_OPT14_HT_L_VAL                 (0x33)
#define BDIC_REG_OPT14_HT_H_VAL                 (0x01)
#define BDIC_REG_OPT15_LT_L_VAL                 (0x11)
#define BDIC_REG_OPT15_LT_H_VAL                 (0x01)
#define BDIC_REG_OPT15_HT_L_VAL                 (0x97)
#define BDIC_REG_OPT15_HT_H_VAL                 (0x01)
#define BDIC_REG_OPT16_LT_L_VAL                 (0x76)
#define BDIC_REG_OPT16_LT_H_VAL                 (0x01)
#define BDIC_REG_OPT16_HT_L_VAL                 (0xEA)
#define BDIC_REG_OPT16_HT_H_VAL                 (0x02)
#define BDIC_REG_OPT17_LT_L_VAL                 (0x51)
#define BDIC_REG_OPT17_LT_H_VAL                 (0x02)
#define BDIC_REG_OPT17_HT_L_VAL                 (0xEC)
#define BDIC_REG_OPT17_HT_H_VAL                 (0x04)
#define BDIC_REG_OPT18_LT_L_VAL                 (0x38)
#define BDIC_REG_OPT18_LT_H_VAL                 (0x04)
#define BDIC_REG_OPT18_HT_L_VAL                 (0x0D)
#define BDIC_REG_OPT18_HT_H_VAL                 (0x08)
#define BDIC_REG_OPT19_LT_L_VAL                 (0xD7)
#define BDIC_REG_OPT19_LT_H_VAL                 (0x06)
#define BDIC_REG_OPT19_HT_L_VAL                 (0xF9)
#define BDIC_REG_OPT19_HT_H_VAL                 (0x0D)
#define BDIC_REG_OPT20_LT_L_VAL                 (0x9E)
#define BDIC_REG_OPT20_LT_H_VAL                 (0x0B)
#define BDIC_REG_OPT20_HT_L_VAL                 (0xB5)
#define BDIC_REG_OPT20_HT_H_VAL                 (0x15)
#define BDIC_REG_OPT21_LT_L_VAL                 (0x04)
#define BDIC_REG_OPT21_LT_H_VAL                 (0x13)
#define BDIC_REG_OPT21_HT_L_VAL                 (0xC6)
#define BDIC_REG_OPT21_HT_H_VAL                 (0x24)
#define BDIC_REG_OPT22_LT_L_VAL                 (0x96)
#define BDIC_REG_OPT22_LT_H_VAL                 (0x1E)
#define BDIC_REG_OPT22_HT_L_VAL                 (0xB6)
#define BDIC_REG_OPT22_HT_H_VAL                 (0x48)
#define BDIC_REG_OPT23_LT_L_VAL                 (0xD6)
#define BDIC_REG_OPT23_LT_H_VAL                 (0x39)
#define BDIC_REG_OPT23_HT_L_VAL                 (0xFF)
#define BDIC_REG_OPT23_HT_H_VAL                 (0xFF)

#define SENSOR_REG_COMMAND2_INI_VAL             (0x28)
#define SENSOR_REG_COMMAND2_ALS_ON_VAL          (0x1C)

#define SENSOR_REG_PS_LT_LSB_VAL                (0xFF)
#define SENSOR_REG_PS_LT_MSB_VAL                (0xFF)
#define SENSOR_REG_PS_HT_LSB_VAL                (0xFF)
#define SENSOR_REG_PS_HT_MSB_VAL                (0xFF)

#define BDIC_WAIT_TIMER_START                   (1 * 1000)
#define BDIC_WAIT_TIMER_STOP                    (3 * 1000)

/* ------------------------------------------------------------------------- */
/* VARIABLES                                                                 */
/* ------------------------------------------------------------------------- */
static const unsigned char shdisp_main_bkl_tbl[SHDISP_BKL_FIX_TBL_NUM] = {
    ,
};

static const unsigned short shdisp_main_bkl_min_tbl[SHDISP_BKL_AUTO_OPT_TBL_NUM] = {
    5,
    5,
    5,
    5,
    11,
    16,
    21,
    26,
    26,
    32,
    42,
    47,
    53,
    58,
    63,
    68,
    79,
    79,
    89,
    121,
    147,
    258,
    511,
    800
};

static const unsigned short shdisp_main_bkl_max_tbl[SHDISP_BKL_AUTO_OPT_TBL_NUM] = {
    37,
    89,
    89,
    132,
    247,
    347,
    463,
    611,
    721,
    889,
    1089,
    1263,
    1474,
    1711,
    1884,
    2111,
    2379,
    2547,
    2689,
    2795,
    2853,
    2926,
    2947,
    2947
};

static unsigned short const shdisp_bdic_bkl_ado_index[SHDISP_BKL_AUTO_OPT_TBL_NUM] = {
    ((BDIC_REG_OPT0_HT_H_VAL << 8)  | BDIC_REG_OPT0_HT_L_VAL),
    ((BDIC_REG_OPT1_HT_H_VAL << 8)  | BDIC_REG_OPT1_HT_L_VAL),
    ((BDIC_REG_OPT2_HT_H_VAL << 8)  | BDIC_REG_OPT2_HT_L_VAL),
    ((BDIC_REG_OPT3_HT_H_VAL << 8)  | BDIC_REG_OPT3_HT_L_VAL),
    ((BDIC_REG_OPT4_HT_H_VAL << 8)  | BDIC_REG_OPT4_HT_L_VAL),
    ((BDIC_REG_OPT5_HT_H_VAL << 8)  | BDIC_REG_OPT5_HT_L_VAL),
    ((BDIC_REG_OPT6_HT_H_VAL << 8)  | BDIC_REG_OPT6_HT_L_VAL),
    ((BDIC_REG_OPT7_HT_H_VAL << 8)  | BDIC_REG_OPT7_HT_L_VAL),
    ((BDIC_REG_OPT8_HT_H_VAL << 8)  | BDIC_REG_OPT8_HT_L_VAL),
    ((BDIC_REG_OPT9_HT_H_VAL << 8)  | BDIC_REG_OPT9_HT_L_VAL),
    ((BDIC_REG_OPT10_HT_H_VAL << 8) | BDIC_REG_OPT10_HT_L_VAL),
    ((BDIC_REG_OPT11_HT_H_VAL << 8) | BDIC_REG_OPT11_HT_L_VAL),
    ((BDIC_REG_OPT12_HT_H_VAL << 8) | BDIC_REG_OPT12_HT_L_VAL),
    ((BDIC_REG_OPT13_HT_H_VAL << 8) | BDIC_REG_OPT13_HT_L_VAL),
    ((BDIC_REG_OPT14_HT_H_VAL << 8) | BDIC_REG_OPT14_HT_L_VAL),
    ((BDIC_REG_OPT15_HT_H_VAL << 8) | BDIC_REG_OPT15_HT_L_VAL),
    ((BDIC_REG_OPT16_HT_H_VAL << 8) | BDIC_REG_OPT16_HT_L_VAL),
    ((BDIC_REG_OPT17_HT_H_VAL << 8) | BDIC_REG_OPT17_HT_L_VAL),
    ((BDIC_REG_OPT18_HT_H_VAL << 8) | BDIC_REG_OPT18_HT_L_VAL),
    ((BDIC_REG_OPT19_HT_H_VAL << 8) | BDIC_REG_OPT19_HT_L_VAL),
    ((BDIC_REG_OPT20_HT_H_VAL << 8) | BDIC_REG_OPT20_HT_L_VAL),
    ((BDIC_REG_OPT21_HT_H_VAL << 8) | BDIC_REG_OPT21_HT_L_VAL),
    ((BDIC_REG_OPT22_HT_H_VAL << 8) | BDIC_REG_OPT22_HT_L_VAL),
    ((BDIC_REG_OPT23_HT_H_VAL << 8) | BDIC_REG_OPT23_HT_L_VAL)
};

static const struct shdisp_bdic_led_color_index shdisp_triple_led_color_index_tbl[SHDISP_TRI_LED_COLOR_TBL_NUM] = {
    {0, 0, 0,  0},
    {1, 0, 0,  1},
    {0, 1, 0,  2},
    {1, 1, 0,  3},
    {0, 0, 1,  4},
    {1, 0, 1,  5},
    {0, 1, 1,  6},
    {1, 1, 1,  7},
    {2, 1, 1,  8},
    {2, 0, 0,  9},
    {0, 2, 0, 10}
};

static const unsigned char shdisp_clrvari_index[SHDISP_COL_VARI_KIND] = {
    SHDISP_HANDSET_COLOR_BLACK,
    SHDISP_HANDSET_COLOR_PINK,
    SHDISP_HANDSET_COLOR_WHITE
};

static const unsigned char shdisp_triple_led_tbl[SHDISP_COL_VARI_KIND][SHDISP_TRI_LED_COLOR_TBL_NUM][3] = {
  {
    { 0x00, 0x00, 0x00 },
    { 0x0C, 0x00, 0x00 },
    { 0x00, 0x0D, 0x00 },
    { 0x07, 0x0E, 0x00 },
    { 0x00, 0x00, 0x17 },
    { 0x0A, 0x00, 0x09 },
    { 0x00, 0x12, 0x09 },
    { 0x06, 0x0C, 0x0F },
    { 0x7F, 0x06, 0x06 },
    { 0x0C, 0x00, 0x00 },
    { 0x00, 0x0D, 0x00 }
  },
  {
    { 0x00, 0x00, 0x00 },
    { 0x0C, 0x00, 0x00 },
    { 0x00, 0x0D, 0x00 },
    { 0x07, 0x0E, 0x00 },
    { 0x00, 0x00, 0x17 },
    { 0x0A, 0x00, 0x09 },
    { 0x00, 0x12, 0x09 },
    { 0x06, 0x0C, 0x0F },
    { 0x7F, 0x06, 0x06 },
    { 0x0C, 0x00, 0x00 },
    { 0x00, 0x0D, 0x00 }
  },
  {
    { 0x00, 0x00, 0x00 },
    { 0x0C, 0x00, 0x00 },
    { 0x00, 0x0D, 0x00 },
    { 0x07, 0x0E, 0x00 },
    { 0x00, 0x00, 0x17 },
    { 0x0A, 0x00, 0x09 },
    { 0x00, 0x12, 0x09 },
    { 0x06, 0x0C, 0x0F },
    { 0x7F, 0x06, 0x06 },
    { 0x0C, 0x00, 0x00 },
    { 0x00, 0x0D, 0x00 }
  }
};

static const unsigned char shdisp_triple_led_anime_tbl[SHDISP_COL_VARI_KIND][SHDISP_TRI_LED_ANIME_2PAGE][SHDISP_TRI_LED_COLOR_TBL_NUM][3] = {
  {
    {
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 }
    },
    {
        { 0x00, 0x00, 0x00 },
        { 0x0C, 0x00, 0x00 },
        { 0x00, 0x0D, 0x00 },
        { 0x07, 0x0E, 0x00 },
        { 0x00, 0x00, 0x17 },
        { 0x0A, 0x00, 0x09 },
        { 0x00, 0x12, 0x09 },
        { 0x06, 0x0C, 0x0F },
        { 0x7F, 0x06, 0x06 },
        { 0x0C, 0x00, 0x00 },
        { 0x00, 0x0D, 0x00 }
    }
  },
  {
    {
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 }
    },
    {
        { 0x00, 0x00, 0x00 },
        { 0x0C, 0x00, 0x00 },
        { 0x00, 0x0D, 0x00 },
        { 0x07, 0x0E, 0x00 },
        { 0x00, 0x00, 0x17 },
        { 0x0A, 0x00, 0x09 },
        { 0x00, 0x12, 0x09 },
        { 0x06, 0x0C, 0x0F },
        { 0x7F, 0x06, 0x06 },
        { 0x0C, 0x00, 0x00 },
        { 0x00, 0x0D, 0x00 }
    }
  },
  {
    {
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 },
        { 0x00, 0x00, 0x00 }
    },
    {
        { 0x00, 0x00, 0x00 },
        { 0x0C, 0x00, 0x00 },
        { 0x00, 0x0D, 0x00 },
        { 0x07, 0x0E, 0x00 },
        { 0x00, 0x00, 0x17 },
        { 0x0A, 0x00, 0x09 },
        { 0x00, 0x12, 0x09 },
        { 0x06, 0x0C, 0x0F },
        { 0x7F, 0x06, 0x06 },
        { 0x0C, 0x00, 0x00 },
        { 0x00, 0x0D, 0x00 }
    }
  }
};

static const struct shdisp_bdic_bkl_ado_tbl shdisp_bdic_bkl_ado_tbl[5] = {
    {   0,      1,  1786,      129},
    {   1,      9,  1140,      609},
    {   9,    146,   677,     5959},
    { 146,   2372,   435,    49052},
    {2372,  65536,   163,   963294},
};

/* ------------------------------------------------------------------------- */
/* TYPES                                                                     */
/* ------------------------------------------------------------------------- */

/* ------------------------------------------------------------------------- */
/* PROTOTYPES                                                                */
/* ------------------------------------------------------------------------- */

#endif /* SHDISP_BL71Y6_DATA_DEFAULT_H */

/* ------------------------------------------------------------------------- */
/* END OF FILE                                                               */
/* ------------------------------------------------------------------------- */
