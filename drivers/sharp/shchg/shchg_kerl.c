/* drivers/sharp/shchg/shchg_kerl.c
 *
...
#define SHCHG_ERROR(x...)	SHPWR_LOG(SHPWR_LOG_LEVEL_ERR,SHPWR_LOG_TYPE_CHG,x)
#define SHCHG_INFO(x...)	SHPWR_LOG(SHPWR_LOG_LEVEL_INFO,SHPWR_LOG_TYPE_CHG,x)
#define SHCHG_TRACE(x...)	SHPWR_LOG(SHPWR_LOG_LEVEL_DEBUG,SHPWR_LOG_TYPE_CHG,x)
...
#define SHCHG_DRV_IOCTL_CMD_GET_TRIM_VALUE								_IOR( SHCHG_IOC_MAGIC, 32, int*)
...
static int shchg_drv_ioctl_cmd_get_trim_value( struct file* fi_p, unsigned long arg );
...
static long shchg_drv_ioctl( struct file* fi_p, unsigned int cmd, unsigned long arg )
{
	...
	else if(cmd == SHCHG_DRV_IOCTL_CMD_GET_TRIM_VALUE)
	{
		ret = shchg_drv_ioctl_cmd_get_trim_value(fi_p,arg);
	}
	else
	{
		ret = shchg_drv_ioctl_cmd_invalid(fi_p,arg);
...
static int shchg_drv_ioctl_cmd_get_trim_value( struct file* fi_p, unsigned long arg )
{
	...
	val[0] = qpnp_chg_get_iusb_trim();
	val[1] = qpnp_chg_get_idc_trim();

	if(copy_to_user((int __user *)arg,val,sizeof(val)) != 0)
	{
		SHCHG_ERROR("%s : copy_to_user failed.\n",__FUNCTION__);
		ret = -EPERM;
	}
