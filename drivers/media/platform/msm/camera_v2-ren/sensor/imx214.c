...
#if defined(CONFIG_MACH_LYNX_DL50) || defined(CONFIG_MACH_LYNX_DL60)
	{
		.seq_type = SENSOR_CLK,
		.seq_val = SENSOR_CAM_MCLK,
		.config_val = 9876500,
...
#elif defined(CONFIG_MACH_DECKARD_AS99) || defined(CONFIG_MACH_DECKARD_AL15)
	{
		.seq_type = SENSOR_CLK,
		.seq_val = SENSOR_CAM_MCLK,
		.config_val = 6000000,
...
#elif defined(CONFIG_MACH_EBZ) || defined(CONFIG_MACH_REN)
	{
		.seq_type = SENSOR_CLK,
		.seq_val = SENSOR_CAM_MCLK,
//		.config_val = 8000000,
...
//	perf_lock_init(&imx214_perf_lock, PERF_LOCK_729600KHz, "camera_imx214");
	...
	if(rev == 0x00){
		...
		rc = i2c_add_driver(&imx214_i2c_driver);
		CDBG("%s:%d rc %d\n", __func__, __LINE__, rc);
		return rc;
	}
#else
